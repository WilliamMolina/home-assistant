...
CONF_SPEAKER_NAME = "speaker_name"

PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    vol.Required(CONF_SPEAKER_NAME): cv.string,
...
def async_setup_platform(hass, config, async_add_devices, discover_info=None):
    ...
    speakername = config.get(CONF_SPEAKER_NAME)
    host = config.get(CONF_HOST)
    name = config.get(CONF_NAME)

    #hass.loop.set_debug(True)
    heos = HeosMediaPlayer(hass, speakername, host, name)
...
class HeosMediaPlayer(MediaPlayerDevice):
    ...
    def __init__(self, hass, speakername, host, name):
        """Initialize"""
        from .aioheos import AioHeos
        if host is None:
            _LOGGER.info('No host provided, will try to discover...')
        self._hass = hass
        self.heos = AioHeos(loop=hass.loop, speakername=speakername, host=host, verbose=True)
        self._name = name
        self._state = None
