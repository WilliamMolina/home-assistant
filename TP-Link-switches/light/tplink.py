...
class TPLinkSmartDimmer(Light):
    """Representation of a TPLink Dimmer Switch."""
    ...
    def turn_on(self, **kwargs):
        ...
        if ATTR_BRIGHTNESS in kwargs:
            self.smartbulb.brightness = brightness_to_percentage(kwargs.get(ATTR_BRIGHTNESS))
    ...
    def update(self):
        ...
        try:
            ...
            if self._ignoreUnavailableCounter <= 2:
                _LOGGER.warning("Could not read state for %s: %s, but ignoring for now (counter=%d)", self._name, ex, self._ignoreUnavailableCounter)
                self._ignoreUnavailableCounter += 1
            else:
                if self._available:
...
class TPLinkSmartBulb(TPLinkSmartDimmer):
    """Representation of a TPLink Smart Bulb."""

    # F821: https://github.com/PyCQA/pyflakes/issues/373 
    def __init__(self, smartbulb: 'SmartBulb', name) -> None:  # noqa: F821
        """Initialize the bulb."""
        TPLinkDimmer.__init__(smartbulb, name)
    ...
    def get_features(self):
        ...
        TPLinkSmartDimmer.get_features(self)
