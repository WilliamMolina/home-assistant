...
class Device(object):
    def __init__(self, url, mac):
        ...
        for x in range(MAX_RETRIES):
            try:
                xml = requests.get(url, timeout=5)
                break
            except Exception as err:
                if x < (MAX_RETRIES - 1):
                    log.warning("Can't initialize device at %s: error (%s), will retry (counter=%d)", url, err, x)
                else:
                    log.error("Can't initialize device at %s: error (%s)", url, err)
                    raise
