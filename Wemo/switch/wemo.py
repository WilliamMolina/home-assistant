...
def service_to_entities(hass, service):
    ...
    if entity_ids:
        entities = [entity for entity in CROCKPOTS
                    if entity.entity_id in entity_ids]
    else:
        entities = list(CROCKPOTS)
...
def setup_platform(hass, config, add_devices_callback, discovery_info=None):
    ...
    if discovery_info is not None:
        ...
        model_name = discovery_info['model_name']
        try:
        ...
        if device:
            if model_name == 'Crockpot':
                crockpot_switch = CrockPot(device)
                CROCKPOTS.append(crockpot_switch)
                add_devices_callback([crockpot_switch])
            else:
                add_devices_callback([WemoSwitch(device)])

    def handle_update_settings(call):
        mode = call.data.get('mode', 0)
        time = call.data.get('time', 0)

        for crockpot in service_to_entities(hass, call):
            crockpot.update_settings(mode, time)

    hass.services.register('wemo', 'crockpot_update_settings', handle_update_settings)
