...
def setup(hass, config):
    ...
    from .pywemo import discovery as discovery_wemo
    from .pywemo import subscribe

    global SUBSCRIPTION_REGISTRY
    SUBSCRIPTION_REGISTRY = subscribe.SubscriptionRegistry()
    ...
    for host, port in config.get(DOMAIN, {}).get(CONF_STATIC, []):
        ...
        try:
            device = discovery_wemo.device_from_description(url, None)
    ...
    if not disable_discovery:
        ...
        devices.extend(
            ...
            for device in discovery_wemo.discover_devices())
